...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, std::vector<int> animationAtlas, EntityType entityType) : 
        mPosition {position}, mVelocity {0.0f, 0.0f}, 
        mAcceleration {0.0f, 0.0f}, mMovement { 0.0f, 0.0f }, mScale {scale},
        mColliderDimensions {scale}, mTexture {LoadTexture(textureFilepath)}, 
        mTextureType {ATLAS}, mSpriteSheetDimensions {spriteSheetDimensions},
        mAnimationAtlas {}, mDirection {RIGHT},
        mAnimationIndices {animationAtlas}, 
        mFrameSpeed {DEFAULT_FRAME_SPEED}, mAngle { 0.0f }, 
        mSpeed { DEFAULT_SPEED }, mEntityType {entityType} { }
...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        //printf("%s", collidableEntity->mEntityType);
        
        if (isColliding(collidableEntity))
        {
            if(collidableEntity->getEntityType() == PLATFORM){
                deactivate();
            }
...
{
    if(mEntityType != PLATFORM) mAnimationIndices = mAnimationAtlas.at(mDirection);
    ...
    int collisionCheckCount)
{
    ...
    if(mEntityType != PLAYER){ mVelocity.x = mMovement.x * mSpeed;}
