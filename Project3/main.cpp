...
void update() 
{
    if(gameState == 0){
        ...
        while (deltaTime >= FIXED_TIMESTEP)
        {
            ...
            if(moving){
                ship->setAcceleration({static_cast<float>(cos(((2 * acos(0.0))/180)*(ship->getAngle()-90)) * ship->getSpeed() * FIXED_TIMESTEP),
                static_cast<float>(sin(((2 * acos(0.0))/180)*(ship->getAngle()-90)) * ship->getSpeed() * FIXED_TIMESTEP)});
            }
