...
Map::Map(int mapColumns, int mapRows, unsigned int *levelData,
         ...
         int textureRows, Vector2 origin, Vector2 boundaries) : 
         ...
         mOrigin {origin}, mBoundaries {boundaries} { build(); }
...
void Map::build()
{
    ...
    mRightBoundary = mBoundaries.x;
    //mTopBoundary    = mOrigin.y - (mMapRows * mTileSize) / 2.0f;
    mTopBoundary = 0.0f;
    //mBottomBoundary = mOrigin.y + (mMapRows * mTileSize) / 2.0f;
    mBottomBoundary = mBoundaries.y;
    mTextureArea = {
        0.0f, 0.0f,
        (float) mTextureAtlas.width,
        (float) mTextureAtlas.height
    };
    ...
    for (int row = 0; row < mMapRows; row++)
    {
        ...
        for (int col = 0; col < mMapColumns; col++)
        {
            ...
            Rectangle destinationArea = {
                col == 4 ? mOrigin.x-(mTileSize/2.0f) : mLeftBoundary + col * mTileSize,//mLeftBoundary + col * mTileSize,
