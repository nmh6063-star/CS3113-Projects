...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mSpriteSheetDimensions {}, mDirection {FACE_FORWARD}, 
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
    ...
    mTextureType {SINGLE}, mDirection {FACE_FORWARD}, mAnimationAtlas {{}}, 
...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        ...
        mAnimationAtlas {animationAtlas}, mDirection {FACE_FORWARD},
        mAnimationIndices {animationAtlas.at(FACE_FORWARD)}, 
        mFrameSpeed {DEFAULT_FRAME_SPEED}, mAngle { 0.0f }, 
        mSpeed { DEFAULT_SPEED }, mEntityType {entityType} { }

Entity::~Entity() 
{ 
    printf("Starting PLAYER shutdown...\n");
    UnloadTexture(mTexture); 
    printf("Ending PLAYER shutdown...\n");
};
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    if(mEntityType == PLAYER){
        setScale(
            {((float) mScale.x) + (deltaTime * 5.0f)*((float)(startScale.x-mScale.x)),
            ((float) mScale.y) + (deltaTime * 5.0f)*((float)(startScale.y-mScale.y))}
        );
    }
    else if(mEntityType == HOOK){
        float angle = 0.0f;
        float offsetX = 0.0f;
        float offsetY = 0.0f;
        ...
        setAngle(mAngle + (deltaTime * 10.0f)*(angle-mAngle));
        setPosition({
            mPosition.x + (deltaTime * 10.0f)*((player->getPosition().x+offsetX)-mPosition.x),
            mPosition.y + (deltaTime * 10.0f)*((player->getPosition().y+offsetY)-mPosition.y)
        });
    }
    else if(mEntityType == ARROW){
        if(arrowPos == 3){
            scoreValue = 1.05f;
        }
        if(arrowPos == 5){
            ...
            setPosition({
                mPosition.x + (deltaTime * 10.0f)*((nextPos.x)-mPosition.x),
                mPosition.y + (deltaTime * 10.0f)*((nextPos.y)-mPosition.y)
            });
