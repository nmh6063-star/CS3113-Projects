...
void LevelA::initialise()
{
   ...
   */
   ...
   mGameState.mimi = new Entity(
      {mOrigin.x - 500.0f, mOrigin.y}, // position
   ...
   mGameState.misc1 = new Entity(
      {(mOrigin.x - (LEVEL_WIDTH * TILE_DIMENSION) / 2.0f) + (12.5f * TILE_DIMENSION), 
         (mOrigin.y - (LEVEL_HEIGHT * TILE_DIMENSION) / 2.0f) + (1.5f * TILE_DIMENSION)}, // position
      {90.0f, 90.0f},             // scale
   ...
   mGameState.misc2 = new Entity({
      ...
      {100.0f * sizeRatio, 100.0f},             // scale
      "assets/game/Slime-Sheet.png",                   // texture file address
      ATLAS,                                    // single image or atlas?
      { 3,  16},                                 // atlas dimensions
      slimeAnimationAtlas,                    // actual atlas
      NPC
   });
   mGameState.misc2->setAIType(WANDERER);
   ...
   mGameState.misc2->setColliderDimensions({
        ...
        mGameState.misc2->getScale().y /1.5f
    });

    mGameState.misc2->setAcceleration({0.0f, ACCELERATION_OF_GRAVITY});
    mGameState.misc2->setDirection(LEFT);
...
void LevelA::update(float deltaTime)
{
   ...
   if(mGameState.misc2->getPosition().x < mOrigin.x - 100.0f){
      mGameState.misc2->setDirection(RIGHT);
   }
   else if(mGameState.misc2->getPosition().x > mOrigin.x + 450.0f){
      mGameState.misc2->setDirection(LEFT);
   }

   Vector2 currentPlayerPosition = { mGameState.mimi->getPosition().x, mOrigin.y };

   if (mGameState.mimi->getPosition().y > 800.0f || mGameState.mimi->checkKill() == true) {mGameState.lives--; mGameState.mimi->setPosition({mOrigin.x - 500.0f, mOrigin.y}); mGameState.mimi->resetKill(); PlaySound(mGameState.deathSound);};
   if (mGameState.mimi->getContinue() == SWAP) mGameState.nextSceneID = 2;
