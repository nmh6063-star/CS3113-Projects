...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        {
            if(collidableEntity->getEntityType() == DOOR){
                if(canContinue != SWAP){
                    setContinue(CONTINUE);
                }
                continue;
            }
            else if(collidableEntity->getEntityType() == PLAYER){
                collidableEntity->killPLayer();
                continue;
            }   
...
void Entity::AIWander() { if(mDirection == LEFT) moveLeft(); else moveRight(); }

void Entity::AIFly(){
    if(mPosition.x < AIStartFly.x && mDirection == LEFT){
        mDirection = RIGHT;
    }
    else if(mPosition.x > AIEndFly.x && mDirection == RIGHT){
        mDirection = LEFT;
    }
    if(mPosition.y > AIStartFly.y && mDirection == LEFT){
        moveDown();
    }
    else if(mPosition.y < AIEndFly.y && mDirection == RIGHT){
        moveUp();
    }
    AIWander();
}
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    if (mTextureType == ATLAS && ((GetLength(mMovement) != 0 && mIsCollidingBottom) || getDirection()==IDLELEFT || getDirection()==IDLERIGHT || mAIType == FLYER || mAIType == FOLLOWER)) 
        animate(deltaTime);
}
