...
void LevelC::initialise()
{
   ...
   */
   ...
   mGameState.misc2 = new Entity({
      {mOrigin.x + 250.0f, mOrigin.y+100.0f}, // position
      {100.0f * sizeRatio, 100.0f},             // scale
   ...
   mGameState.misc2->setAIType(FOLLOWER);
   mGameState.misc2->setAIState(IDLE);
   mGameState.misc2->setSpeed(Entity::DEFAULT_SPEED * 0.5f);
...
void LevelC::update(float deltaTime)
{
   ...
   if (mGameState.mimi->getPosition().y > 800.0f || mGameState.mimi->checkKill() == true) {mGameState.lives--; mGameState.mimi->setPosition({mOrigin.x - 500.0f, mOrigin.y}); mGameState.misc2->setPosition({mOrigin.x + 250.0f, mOrigin.y+100.0f}); mGameState.mimi->resetKill(); PlaySound(mGameState.deathSound);};
   if (mGameState.mimi->getContinue() == SWAP) mGameState.nextSceneID = 5;
